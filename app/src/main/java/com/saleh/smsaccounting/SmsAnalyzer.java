...
public class SmsAnalyzer{
 public static class Result{String type="unknown",party="",reference="",error="";double amount=0,smsBalance=0;}
 ...
 public static Result analyze(String raw){
  Result r=new Result();String s=digits(raw==null?"":raw);
  ...
  if(r.type.equals("debit")||r.type.equals("credit")){String[] p=s.split("/");if(p.length>1)r.party=p[1].trim();}
  if(r.type.equals("unknown"))r.error="لم يتم التعرف على نوع العملية";
  else if(r.amount<=0)r.error="لم يتم العثور على مبلغ صحيح";
  return r;
 }
}